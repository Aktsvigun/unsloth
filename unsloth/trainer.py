...
class UnslothTrainingArguments(TrainingArguments):
    """
    Dataclass containing training arguments for the UnslothTrainer.

    Args:
            embedding_learning_rate (`float`, *optional*):
                    Learning rate specifically for embedding layers. If not provided, uses the default learning rate from TrainingArguments.
    """
...
def _create_unsloth_optimizer(
    ...
    """
    Creates an optimizer with separate learning rates for embeddings and other parameters.

    Args:
            model (`torch.nn.Module`):
                    The model whose parameters will be optimized.
            optimizer_cls (`type[torch.optim.Optimizer]`):
                    The class of optimizer to use (e.g., AdamW).
            optimizer_kwargs (`dict[str, Any]`):
                    Keyword arguments for the optimizer constructor.
            embedding_lr (`float`):
                    Learning rate to use for embedding layers.

    Returns:
            `torch.optim.Optimizer`: An optimizer instance with separate learning rates for embeddings and other parameters.
    """
...
class UnslothTrainer(SFTTrainer):
    """
    A trainer class that extends SFTTrainer with Unsloth-specific optimizations, including separate learning rates for embeddings.

    This trainer is designed to work with Unsloth's efficient fine-tuning methods and provides better performance for certain model architectures.
    """
    ...
    def create_optimizer(self) -> torch.optim.Optimizer:
        ...
        If an embedding learning rate is specified in the training arguments, this method creates an optimizer with different learning rates for embedding layers and other parameters.

        Returns:
                `torch.optim.Optimizer`: The created optimizer with appropriate learning rate groupings.
        """
...
def _backwards_compatible_trainer(trainer_class: type, config_class: type) -> type:
    """
    Makes a trainer class backwards compatible with older versions of TRL by handling parameter passing differences.

    This function wraps the trainer's __init__ method to handle the transition between TRL versions where some parameters were moved from TrainingArguments to specific config classes.
    ...
    Returns:
            `type`: A modified trainer class with backwards compatible initialization.
    """
...
def _patch_trl_trainer() -> None:
    """
    Patches TRL trainer classes to ensure backwards compatibility across different versions.

    This function identifies all trainer classes in the TRL library and applies the backwards compatibility patch to handle parameter passing differences between TRL versions.
    """
    import trl
