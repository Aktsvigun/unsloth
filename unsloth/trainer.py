...
class UnslothTrainingArguments(TrainingArguments):
    """
    Training arguments specifically for Unsloth which allows for separate learning rates for embeddings and lm_head.
    """
...
def _create_unsloth_optimizer(
    ...
    """
    Creates an optimizer with separate learning rates for embeddings and non-embedding parameters.
...
class UnslothTrainer(SFTTrainer):
    """
    A trainer class that uses the Unsloth optimizer with separate learning rates for embeddings and lm_head.
    """

    def create_optimizer(self) -> torch.optim.Optimizer:
        """
        Creates an optimizer with separate learning rates for embeddings and non-embedding parameters if an embedding learning rate is specified. Otherwise, uses the default optimizer creation method.
        """
...
def _backwards_compatible_trainer(trainer_class: type, config_class: type) -> type:
    """
    Makes a trainer class backwards compatible with older versions of TRL by handling parameter passing changes.

    Args:
            trainer_class (`type`):
                    The trainer class to make backwards compatible.
            config_class (`type`):
                    The config class associated with the trainer.

    Returns:
            `type`: A new trainer class with backwards compatibility.
    """
...
def _patch_trl_trainer() -> None:
    """
    Patches TRL trainers to be backwards compatible with Unsloth by modifying their initialization methods.
    """
    import trl
