...
from typing import Optional, Callable, Union, List, Tuple, T, Dict, Any
import sys
import requests
import torch
import os
...
def print_quantization_methods() -> None:
    """
    Prints all available quantization methods along with their descriptions.
    """
...
def check_if_sentencepiece_model(
    model, temporary_location: str = "_unsloth_sentencepiece_temp"
) -> bool:
    """
    Checks if the given model uses SentencePiece tokenizer.

    Args:
            model: The model to check
            temporary_location (str): Temporary directory path for checking
...
def _free_cached_model(model) -> None:
    """
    Frees cached model from disk to save space.

    Args:
            model: The model whose cache needs to be freed
    """
...
def fast_save_pickle(shard, name: str) -> None:
    """
    Quickly saves data using torch.save with pickle.
...
def unsloth_save_model(
    ...
    """
    Saves model with various options (LoRA, merged 16bit, merged 4bit).

    Args:
            model: Model to save
            tokenizer: Associated tokenizer
            save_directory (str): Path to save directory
            save_method (str): Saving method ('lora', 'merged_16bit', 'merged_4bit')
            push_to_hub (bool): Whether to push to HuggingFace Hub
            token (str): HuggingFace auth token
            is_main_process (bool): Whether main process
            state_dict (dict): Model state dict
            save_function: Saving function
            ...
            temporary_location (str): Temp dir path
            maximum_memory_usage (float): Max memory usage
...
def install_llama_cpp_clone_non_blocking() -> subprocess.Popen:
    """
    Clones llama.cpp repo non-blocking.
...
def install_llama_cpp_make_non_blocking() -> Tuple[subprocess.Popen, bool]:
    """
    Compiles llama.cpp non-blocking.

    Returns:
            tuple: (Process handle, whether using CMake)
    """
...
def install_python_non_blocking(packages: List[str] = []) -> subprocess.Popen:
    """
    Installs Python packages non-blocking.
...
def try_execute(commands: List[str], force_complete: bool = False) -> Optional[str]:
    ...
    Args:
            ...
            force_complete (bool): Raise on error

    Returns:
            str: 'CMAKE' if cmake detected, else None
    """
...
def install_llama_cpp_old(version: int = -10) -> None:
    ...
    Args:
            version (int): Version index to install
    """
...
def get_executable(executables: List[str]) -> Optional[str]:
    ...
    Args:
            executables (list): Executable names to search

    Returns:
            str: Path to executable if found
    """
...
def save_to_gguf(
    ...
    quantization_method: Union[
        str, List[str]
    ] = "fast_quantized",  # Can be a list of options! ["q4_k_m", "q8_0", "q5_k_m"]
    first_conversion: str = None,
    _run_installer: Optional[
        Tuple[subprocess.Popen, bool]
    ] = None,  # Non blocking install of llama.cpp
) -> Tuple[List[str], bool]:
    """
    ...
    Args:
            ...
            is_sentencepiece (bool): Uses SentencePiece
            model_directory (str): Model directory
            quantization_method (str/list): Quant method(s)
            first_conversion (str): Initial conversion type
            _run_installer: Installer process

    Returns:
            tuple: (List of saved files, whether full precision kept)
    """
    # logger.warning(
    ...
    quantize_location = get_executable(
        ["llama-quantize", "quantize", "llama-quantize.exe", "quantize.exe"]
    )
    convert_location = get_executable(
    ...
    final_location = str(
        (Path(model_directory) / f"unsloth.{first_conversion.upper()}.gguf").absolute()
    )
    ...
    if use_fast_convert:
        ...
        command = (
            f"python {convert_location} {model_directory} "
    ...
    for quant_method in quantization_method:
        if quant_method != first_conversion:
            print(
                f"Unsloth: [2] Converting GGUF 16bit into {quant_method}. This might take 20 minutes..."
            )
            final_location = str(
                (
                    Path(model_directory) / f"unsloth.{quant_method.upper()}.gguf"
                ).absolute()
            )

            command = (
                f"./{quantize_location} {full_precision_location} "
...
def _determine_username(
    save_directory: str, old_username: Optional[str], token: Optional[str]
) -> Tuple[str, str]:
    """
    Determines HF username from save path.

    Args:
            save_directory (str): Save path
...
def create_huggingface_repo(
    ...
    """
    Creates HF repo for model.

    Args:
            model: The model
            save_directory (str): Repo path
            token (str): HF auth token
            private (bool): Make private

    Returns:
            tuple: (Repo path, HF API instance)
    """
    if token is None:
        token = get_token()
    pass
...
def upload_to_huggingface(
    ...
    Args:
            ...
            old_username (str): Previous username
            private (bool): Make private
            create_config (bool): Create config
...
def fix_tokenizer_bos_token(tokenizer) -> Tuple[bool, Optional[str]]:
    ...
    Returns:
            tuple: (Whether fixed, original template)
    """
...
def create_ollama_model(
    username: str, model_name: str, tag: str, modelfile_path: str
) -> str:
    """
    Creates Ollama model from GGUF.

    Args:
            username (str): Ollama username
            model_name (str): Model name
            tag (str): Model tag
            modelfile_path (str): Modelfile path
...
def push_to_ollama(
    tokenizer, gguf_location: str, username: str, model_name: str, tag: str
) -> None:
    """
    Full pipeline to push to Ollama.
...
def save_lora_to_custom_dir(model, tokenizer, save_directory: str) -> None:
    ...
    Args:
            ...
            tokenizer: Tokenizer
            save_directory (str): Custom directory path
    """
...
def unsloth_convert_lora_to_ggml_and_push_to_hub(
    ...
    Args:
            ...
            maximum_memory_usage (float): Max memory
    """
    if not os.path.exists("llama.cpp"):
        if IS_KAGGLE_ENVIRONMENT:
...
def unsloth_generic_save(
    ...
    Args:
            ...
            tokenizer: Tokenizer
            save_directory (str): Save path
            save_method (str): Save method
            push_to_hub (bool): Push to HF
            token (str): HF token
            is_main_process (bool): Main process
            state_dict (dict): State dict
            save_function: Save function
            max_shard_size: Shard size
            safe_serialization (bool): Safe serialization
            variant: Model variant
...
def not_implemented_save(*args, **kwargs) -> None:
    """
    Placeholder for unimplemented save.
...
def patch_saving_functions(model, vision: bool = False):
    ...
    Args:
            ...
            vision (bool): Whether vision model
