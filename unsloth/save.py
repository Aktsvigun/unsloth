...
def check_if_sentencepiece_model(model, temporary_location: str = "_unsloth_sentencepiece_temp") -> bool:
	"""
	Checks if the model uses SentencePiece tokenizer.
	
	Args:
		model: The model to check
		temporary_location (str): Temporary directory to check tokenizer files
	
	Returns:
		bool: True if model uses SentencePiece tokenizer
	"""
...
def _merge_lora(layer, name: str) -> tuple[torch.Tensor, Optional[torch.Tensor]]:
	...
	Returns:
		tuple: Merged weights tensor and bias tensor if exists
	"""
...
def fast_save_pickle(shard, name: str) -> None:
	"""
	Quickly saves a shard using torch.save with basic settings.
...
def unsloth_save_model(
	...
	"""
	Saves a model with various options for quantization and merging.
	
	Args:
		model: Model to save
		tokenizer: Associated tokenizer
		save_directory (str): Where to save the model
		save_method (str): ['lora', 'merged_16bit', 'merged_4bit']
		push_to_hub (bool): Whether to push to HuggingFace Hub
		token (str): HF auth token
		is_main_process (bool): For distributed training
		state_dict (dict): Custom state dict
		save_function (callable): How to save tensors
		max_shard_size (str/int): Max size per shard
		safe_serialization (bool): Use safe serialization
		variant (str): Model variant
		save_peft_format (bool): Save in PEFT format
		use_temp_dir (bool): Use temp dir for upload
		commit_message (str): Commit message
		private (bool): Make repo private
		create_pr (bool): Create PR
		revision (str): Git revision
		commit_description (str): Longer description
		tags (list): Model tags
		temporary_location (str): Temp dir for intermediates
		maximum_memory_usage (float): Max RAM to use (0-1)
	
	Returns:
		tuple: (save_directory, username)
	"""
...
def install_llama_cpp_clone_non_blocking() -> subprocess.Popen:
	"""
	Clones llama.cpp repo in background.
...
def install_llama_cpp_make_non_blocking() -> tuple[subprocess.Popen, bool]:
	"""
	Compiles llama.cpp in background.
	
	Returns:
		tuple: (subprocess.Popen, bool) where bool indicates if cmake was used
	"""
...
def install_python_non_blocking(packages: List[str] = []) -> subprocess.Popen:
	"""
	Installs Python packages in background.
...
def try_execute(commands: List[str], force_complete: bool = False) -> Optional[str]:
	"""
	Attempts to run shell commands with error handling.
	
	Args:
		commands (list): Commands to run
		force_complete (bool): Raise errors if commands fail
	
	Returns:
		str/None: 'CMAKE' if cmake detected, else None
	"""
...
def install_llama_cpp_old(version: int = -10) -> None:
	"""
	Installs older version of llama.cpp as fallback.
	
	Args:
		version (int): Which old version to install (-10 = 10th latest)
	"""
...
def install_llama_cpp_blocking(use_cuda: bool = False) -> None:
	"""
	Installs llama.cpp synchronously.
	
	Args:
		use_cuda (bool): Whether to enable CUDA support
	"""
	# https://github.com/ggerganov/llama.cpp/issues/7062
	# Weirdly GPU conversion for GGUF breaks??
...
def get_executable(executables: List[str]) -> Optional[str]:
	"""
	Finds executable in system PATH.
	
	Args:
		executables (list): Possible executable names
	
	Returns:
		str/None: Path to executable if found
	"""
...
def save_to_gguf(
	...
	_run_installer: Optional[tuple[subprocess.Popen, bool]] = None, # Non blocking install of llama.cpp
) -> tuple[List[str], bool]:
	"""
	Converts model to GGUF format with quantization options.
	
	Args:
		model_type (str): Model architecture type
		model_dtype (str): Original model dtype
		is_sentencepiece (bool): Uses SentencePiece tokenizer
		model_directory (str): Where model is saved
		quantization_method (str/list): Quantization type(s)
		first_conversion (str): Initial conversion type
		_run_installer: Background installer process
	
	Returns:
		tuple: (list of output files, bool if full precision kept)
	"""
	# logger.warning(
...
def _determine_username(save_directory: str, old_username: Optional[str], token: Optional[str]) -> tuple[str, str]:
	"""
	Determines HF username from save directory.
	
	Args:
		save_directory (str): Target save path
		old_username (str): Previous username if any
		token (str): HF auth token
	
	Returns:
		tuple: (full save path, username)
	"""
	username = ""
...
def create_huggingface_repo(
	...
	"""
	Creates a new HF repo for the model.
	
	Args:
		model: The model
		save_directory (str): Repo name
		token (str): HF auth token
		private (bool): Make repo private
	
	Returns:
		tuple: (repo_id, HfApi instance)
	"""
	if token is None :
		token = get_token()
	pass
...
def upload_to_huggingface(
	...
	"""
	Uploads files to HF Hub.
	
	Args:
		model: The model
		save_directory (str): Repo name
		token (str): HF auth token
		method (str): Upload description
		extra (str): Additional tag
		file_location (str): File to upload
		old_username (str): Previous username
		private (bool): Make repo private
		create_config (bool): Create config.json
...
def fix_tokenizer_bos_token(tokenizer) -> tuple[bool, Optional[str]]:
	"""
	Handles BOS token in tokenizer templates.
	
	Args:
		tokenizer: The tokenizer
	
	Returns:
		tuple: (bool if fixed, original template)
	"""
...
def create_ollama_model(
	...
	"""
	Creates an Ollama model from GGUF file.
	
	Args:
		username (str): Owner name
		model_name (str): Model name
		tag (str): Version tag
		modelfile_path (str): Path to modelfile
...
def push_to_ollama(
	...
	"""
	Full pipeline to create and push Ollama model.
	
	Args:
		tokenizer: The tokenizer
		gguf_location (str): GGUF file path
		username (str): Owner name
		model_name (str): Model name
		tag (str): Version tag
	"""
...
def save_lora_to_custom_dir(model, tokenizer, save_directory: str) -> None:
	"""
	Saves LoRA adapters to custom directory.
	
	Args:
		model: The model
		tokenizer: The tokenizer
		save_directory (str): Target directory
	"""
...
def unsloth_convert_lora_to_ggml_and_push_to_hub(
	...
	"""
	Converts LoRA to GGML and pushes to HF Hub.
	
	Args:
		self: The model
		tokenizer: The tokenizer
		repo_id (str): HF repo name
		use_temp_dir (bool): Use temp dir
		commit_message (str): Commit message
		private (bool): Make private
		token (str): HF auth token
		create_pr (bool): Create PR
		revision (str): Git revision
		commit_description (str): Description
		temporary_location (str): Temp dir
		maximum_memory_usage (float): Max RAM (0-1)
	"""
	if not os.path.exists("llama.cpp"):
		if IS_KAGGLE_ENVIRONMENT:
...
def save_to_gguf_generic(
	...
	"""
	Generic GGUF conversion for non-Llama models.
	
	Args:
		model: The model
		save_directory (str): Target directory
		quantization_type (str): Quant method
		repo_id (str): HF repo name
		token (str): HF auth token
...
def unsloth_generic_save(
	...
	"""
	Generic model saving with various options.
	...
	Args:
		...
		save_directory (str): Target directory
		save_method (str): ['lora', 'merged_16bit', 'merged_4bit']
		push_to_hub (bool): Push to HF Hub
		token (str): HF auth token
		is_main_process (bool): Distributed training
		state_dict (dict): Custom state
		save_function (callable): Save method
		max_shard_size (str/int): Shard size
		safe_serialization (bool): Safe save
		variant (str): Model variant
		save_peft_format (bool): PEFT format
		use_temp_dir (bool): Temp dir
		commit_message (str): Commit msg
		private (bool): Private repo
		create_pr (bool): Create PR
		revision (str): Git revision
		commit_description (str): Description
		tags (list): Model tags
...
def not_implemented_save(*args, **kwargs) -> None:
	"""
	Placeholder for unimplemented save methods.
...
def patch_saving_functions(model, vision: bool = False):
	"""
	Patches model with custom save methods.
	
	Args:
		model: Model to patch
		vision (bool): Is vision model
