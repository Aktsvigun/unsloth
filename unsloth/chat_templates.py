...
def _change_system_message(
    ...
    """
    Replaces the {system_message} placeholder in a chat template with either a provided system message or the default for that template type.

    Args:
            template (`str`): The chat template string containing {system_message} placeholder
            type_chat_template (`str`): The type/name of the chat template being used
            system_message (`str`, optional): Custom system message to use instead of default

    Returns:
            `tuple[str, str]`: (updated_template, final_system_message_used)
    """
...
def get_chat_template(
    ...
    """
    Configures a tokenizer with a specified chat template format and handles special token mappings.

    Args:
            tokenizer (`PreTrainedTokenizer`): The tokenizer to configure
            chat_template (`str` or `tuple`): Either a template name or (template_str, eos_token) tuple
            mapping (`dict`): Mapping dictionary for role/content field names
            map_eos_token (`bool`): Whether to map template's EOS token to tokenizer's EOS
            system_message (`str`, optional): Custom system message for the template
...
def remove_special_tokens(tokenizer: PreTrainedTokenizer, prompt: str) -> str:
    """
    Removes leading BOS (beginning of sequence) tokens from a prompt string.

    Args:
            tokenizer (`PreTrainedTokenizer`): Tokenizer containing BOS token info
            prompt (`str`): Input prompt string that may start with BOS token

    Returns:
            `str`: Prompt with leading BOS token removed if present
    """
...
def _parse_combined_prompt(
    combined_prompt: str, dataset: Dataset
) -> tuple[list[str], list[Union[str, tuple[tuple[int, int], str]]]]:
    """
    Parses a combined prompt string containing column references and optional sections.

    Args:
            combined_prompt (`str`): The prompt string containing {column} references and [[optional]] sections
            dataset (`Dataset`): Dataset to validate column references against

    Returns:
            `tuple[list[str], list]`: (list of column names found, list of prompt segments with optional spans marked)
    """
    # Find {...}
...
def _create_formatter(
    ...
    """
    Generates Python code to format dataset examples according to a parsed prompt structure.

    Args:
            possible_columns (`list[str]`): Column names found in prompt
            final_optional_prompts (`list`): Parsed prompt segments from _parse_combined_prompt
            user_column_name (`str`): Name for final output column

    Returns:
            `str`: Python function code that processes examples into formatted prompts
    """
    # Start final prompt!
...
def get_ollama_eos_tokens(
    tokenizer: PreTrainedTokenizer, extra_eos_tokens: list[str] = []
) -> list[str]:
    """
    Identifies potential end-of-sequence tokens from a tokenizer's vocabulary for Ollama model configuration.

    Args:
            tokenizer (`PreTrainedTokenizer`): Tokenizer to analyze
            extra_eos_tokens (`list[str]`, optional): Additional EOS tokens to include

    Returns:
            `list[str]`: List of identified EOS tokens filtered from vocabulary
    """
...
def test_construct_chat_template() -> None:
    """
    Tests the construct_chat_template function by comparing its output to known correct templates.
    ...
    Raises:
            AssertionError: If generated template doesn't match expected output
    """
    token = "hf_"
...
def create_stopping_criteria(
    tokenizer: PreTrainedTokenizer, stop_word: str = "eos_token"
) -> StoppingCriteriaList:
    """
    Creates stopping criteria for text generation based on a specified stop token.

    Args:
            tokenizer (`PreTrainedTokenizer`): Tokenizer containing vocabulary
            stop_word (`str`): Token or phrase that should stop generation (default 'eos_token')
...
def test_chat_templates() -> None:
    """
    Tests various chat templates against known reference implementations.
    ...
    Raises:
            AssertionError: If any template doesn't produce expected output
    """
    messages = [
        {
            "role": "system",
...
def test_hf_gguf_equivalence(
    tokenizer: PreTrainedTokenizer, gguf_model: str = "./model-unsloth.F16.gguf"
) -> bool:
    """
