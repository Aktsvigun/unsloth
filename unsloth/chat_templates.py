...
def _change_system_message(
    ...
    """
    Replaces the {system_message} placeholder in a chat template with either a provided system message or the default for the template type.

    Args:
            template (`str`): The chat template string containing {system_message} placeholder
            type_chat_template (`str`): The type of chat template being used (e.g. 'unsloth', 'zephyr')
            system_message (`str`, optional): Custom system message to use instead of default

    Returns:
            `tuple[str, str]`: The modified template and the system message used
    """
...
def get_chat_template(
    ...
    """
    Configures a tokenizer with a specified chat template and handles token mapping.

    Args:
            tokenizer (`PreTrainedTokenizer`): The tokenizer to configure
            chat_template (`str` or `tuple`): Either a template name or (template, eos_token) tuple
            mapping (`dict`): Mapping dictionary for role/content fields
            map_eos_token (`bool`): Whether to map the template's EOS token to the tokenizer's
...
def remove_special_tokens(tokenizer: PreTrainedTokenizer, prompt: str) -> str:
    ...
    Returns:
            `str`: The prompt with any leading BOS token removed
    """
...
def _parse_combined_prompt(
    ...
    Args:
            combined_prompt (`str`): The prompt string to parse
            dataset (`Dataset`): Dataset to validate column references against

    Returns:
            `tuple[list[str], list]`: List of column names found and parsed prompt segments
    """
    # Find {...}
...
def _create_formatter(
    ...
    """
    Creates a Python function string to format dataset examples according to a parsed prompt template.

    Args:
            possible_columns (`list[str]`): List of column names referenced in template
            final_optional_prompts (`list`): Parsed prompt segments from _parse_combined_prompt
            user_column_name (`str`): Name for the output column

    Returns:
            `str`: Python function string that processes dataset examples
    """
    # Start final prompt!
...
def get_ollama_eos_tokens(
    ...
    """
    Extracts potential end-of-sequence tokens from a tokenizer's vocabulary for Ollama stopping criteria.

    Args:
            tokenizer (`PreTrainedTokenizer`): Tokenizer to analyze
            extra_eos_tokens (`list[str]`): Additional tokens to include

    Returns:
            `list[str]`: List of potential EOS tokens
    """
...
def test_construct_chat_template() -> None:
    """
    Tests that construct_chat_template produces templates matching known good implementations.
...
def create_stopping_criteria(
    ...
    """
    Creates stopping criteria for generation based on a stop word.

    Args:
            tokenizer (`PreTrainedTokenizer`): Tokenizer for token ID conversion
            stop_word (`str`): The word/phrase to stop generation on

    Returns:
            `StoppingCriteriaList`: Configured stopping criteria
    """
...
def test_chat_templates() -> None:
    """
    Tests that all built-in chat templates produce expected outputs.
