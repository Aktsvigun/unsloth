...
class HideLoggingMessage(logging.Filter):
    ...
    def filter(self, x: logging.LogRecord) -> bool:
        """
        Determines whether a log message should be filtered out based on the presence of a specific text.
        """
...
def extract_approx_params_from_config(config) -> Optional[int]:
    """
...
def patch_mistral_nemo_config(config: str) -> str:
    """
    Patch the Mistral Nemo configuration to add head_dim parameter handling.
    """
...
def is_big_gpu(index: int) -> bool:
    """
    Check if a GPU has sufficient compute capability to use max_autotune_gemm mode.
    """
...
def torch_compile_kwargs(*args, **kwargs) -> dict:
    """
    Returns torch.compile kwargs for accelerate.
    """
...
def patch_regional_compilation() -> None:
    """
    Patch torch.nn.ModuleList to enable regional compilation.
    """
...
def prepare_model_for_kbit_training(
    ...
    """
    Prepare a model for kbit training with specific configurations.
    """
...
def _get_statistics(
    ...
    """
    Internal function to gather and report environment statistics.
    """
...
def get_statistics() -> None:
    """
    Public function to gather and report environment statistics.
    """
    # We log some basic stats about which environment is being used.
    # We simply download a README.md file from HF - all data is made public.
    # This is simply so we can check if some envs are broken or not.
...
def offload_to_disk(
    ...
    """
    Offload a model component to disk to save memory.
    """
...
def offload_input_embeddings(
    model: torch.nn.Module, temporary_location: str = "_unsloth_temporary_saved_buffers"
) -> None:
    """
    Offload input embeddings to disk.
    """
...
def offload_output_embeddings(
    ...
    """
    Offload output embeddings to disk.
    """
    offloaded_W = offload_to_disk(
...
def patch_linear_scaling(
    ...
    """
    Patch model to support linear RoPE scaling.
    """
...
def patch_llama_rope_scaling(
    ...
    """
    Patch Llama model to support various RoPE scaling types.
    """
    assert (
...
def create_boolean_mask(n: int = 4096, sliding_window: int = 2048) -> torch.Tensor:
    """
    Create a boolean attention mask for causal or sliding window attention.
    """
...
def test_mask_creation() -> None:
    """
    Test function to verify boolean mask creation against reference implementation.
    """
...
def _unsloth_pre_compute_loss(self, model, inputs: dict, *args, **kwargs):
    """
    Pre-compute loss with special handling for gradient accumulation.
    """
...
def patch_gradient_accumulation_fix(Trainer) -> None:
    """
    Fix gradient accumulation behavior in training step.
    """
...
def patch_tokenizer(model, tokenizer) -> tuple[Any, Any]:
    """
    Patch tokenizer to work with Unsloth optimizations.
    """
...
def patch_fast_lora() -> None:
    """
    Apply fast LoRA forward pass optimization.
    """
...
def unsloth_compile_transformers(
    ...
    """
    Compile transformers with optimizations for specific model types.
    """
    if Version(torch_version) < Version("2.4.0"):
        print(
            "=" * 30
...
def raise_logits_error(*args, **kwargs) -> None:
    """
    Raise error when empty logits are accessed.
    """
...
def return_none(*args, **kwargs) -> None:
    """
    Return None for unsupported operations on empty logits.
    """
    return None
...
class EmptyLogits:
    """
    Class representing empty logits with error handling for access attempts.
    """
    ...
    def raise_getattr_error(self, attr: str):
        """
        Handle attribute access attempts on empty logits.
        """
    ...
    def __repr__(self) -> str:
        """
        String representation of empty logits warning.
        """
