...
import torch
from typing import Union, Optional, List, Any, Callable, Tuple, T, NoReturn, Type
...
class HideLoggingMessage(logging.Filter):
    """
    A logging filter class that hides log messages containing a specific text.
    """
    ...
    def filter(self, x: logging.LogRecord) -> bool:
        """
        Determines whether a log record should be logged. Returns False if the log message contains the specified text, effectively filtering it out.
        """
...
def extract_approx_params_from_config(config: PretrainedConfig) -> Optional[int]:
    ...
    cleaned = re.sub(
        r"[-_]?bnb[-_]?4bit|[-_]?4bit|[-_]?8bit|[-_]?bnb",
        "",
        model_name,
        flags=re.IGNORECASE,
    )  # replace bnb and xbit
    match_B = re.search(
        r"([0-9]+(?:\.[0-9]+)?)\s*B", cleaned
    )  # first prefer searching 'B'
    if match_B:
...
def patch_mistral_nemo_config(config: str) -> str:
    """
    Patches the Mistral Nemo configuration to add head_dim parameter handling for compatibility with newer versions.
    """
...
def is_big_gpu(index: int) -> bool:
    """
    Checks if a GPU has sufficient compute capability (SM count) to use max_autotune_gemm mode.
    """
...
def torch_compile_kwargs(*args, **kwargs) -> dict:
    """
    Returns torch.compile kwargs for accelerate. Enables dynamic compilation with specific options.
    """
...
def patch_regional_compilation() -> None:
    """
    Patches torch.nn.ModuleList to enable regional compilation for torch 2.5+.
    """
...
def prepare_model_for_kbit_training(
    ...
    """
    Prepares a model for kbit training by setting up gradient checkpointing and other parameters.
    """
...
def _get_statistics(
    statistics: Optional[str] = None, force_download: bool = True
) -> None:
    """
    Internal function to gather environment statistics for usage tracking.
    """
...
def get_statistics() -> None:
    """
    Public function to gather and report environment statistics for usage tracking.
    """
    # We log some basic stats about which environment is being used.
    # We simply download a README.md file from HF - all data is made public.
    # This is simply so we can check if some envs are broken or not.
...
def offload_to_disk(
    ...
    """
    Offloads a model weight to disk to save GPU memory.
    """
...
def offload_input_embeddings(
    model: nn.Module, temporary_location: str = "_unsloth_temporary_saved_buffers"
) -> None:
    """
    Offloads input embeddings to disk.
    """
...
def offload_output_embeddings(
    ...
    """
    Offloads output embeddings to disk.
    """
    offloaded_W = offload_to_disk(
...
def is_bfloat16_supported() -> bool:
    """
    Checks if bfloat16 is supported on the current hardware.
    """
...
def is_vLLM_available() -> bool:
    """
    Checks if vLLM is available in the current environment.
    """
...
def patch_linear_scaling(
    ...
    rope_module: Optional[Type] = None,
    scaled_rope_module: Optional[Type] = None,
    attention_module: Optional[Type] = None,
) -> tuple[Optional[str], Optional[str]]:
    """
    Patches model attention layers to support linear RoPE scaling.
    """
...
def patch_llama_rope_scaling(
    ...
    rope_module: Optional[Type] = None,
    scaled_rope_module: Optional[Type] = None,
    extended_rope_module: Optional[Type] = None,
    attention_module: Optional[Type] = None,
    longrope_module: Optional[Type] = None,
) -> tuple[Optional[str], Optional[str]]:
    """
    Patches Llama model attention layers to support various RoPE scaling types.
    """
    assert (
...
def create_boolean_mask(n: int = 4096, sliding_window: int = 2048) -> torch.Tensor:
    """
    Creates a boolean attention mask for causal or sliding window attention.
    """
...
def test_mask_creation() -> None:
    """
    Tests the boolean mask creation against HuggingFace's implementation.
    """
...
def _unsloth_pre_compute_loss(
    self, model: nn.Module, inputs: dict[str, torch.Tensor], *args, **kwargs
) -> torch.Tensor:
    """
    Pre-computes loss with special handling for gradient accumulation.
    """
...
def patch_gradient_accumulation_fix(Trainer: Type[Trainer]) -> None:
    """
    Fixes gradient accumulation behavior in the Trainer class.
    """
...
def patch_tokenizer(
    model: nn.Module, tokenizer: PreTrainedTokenizer
) -> tuple[nn.Module, PreTrainedTokenizer]:
    """
    Patches tokenizer to work with Unsloth optimizations.
    """
...
def patch_fast_lora() -> None:
    """
    Patches LoRA forward pass for faster execution.
    """
...
def unsloth_compile_transformers(
    ...
    """
    Configures and applies torch.compile optimizations to transformer models.
    """
    if Version(torch_version) < Version("2.4.0"):
        print(
            "=" * 30
...
def raise_logits_error(*args, **kwargs) -> NoReturn:
    """
    Raises an error when attempting to access empty logits.
    """
...
def return_none(*args, **kwargs) -> None:
    """
    Returns None for empty logits operations.
    """
    return None
...
class EmptyLogits:
    """
    A placeholder class representing empty logits when return_logits=False.
    """

    def __init__(self):
        return

    def raise_getattr_error(self, attr: str):
        """
        Handles attribute access on empty logits by raising an error.
        """
    ...
    def __repr__(self) -> str:
        """
        Returns error message when empty logits are accessed.
        """
