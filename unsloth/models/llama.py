...
import math
import functools
from typing import Optional, Tuple, List, Union, T, Dict, Any, Callable
from ._utils import *
...
def original_apply_qkv(
    ...
    """
    Applies the query, key, and value projections to the input tensor.

    Args:
            X (`torch.Tensor`): Input tensor to apply projections to.

    Returns:
            Tuple[`torch.Tensor`, `torch.Tensor`, `torch.Tensor`]: Tuple containing query, key, and value projections.
    """
    Q = self.q_proj(X)
    K = self.k_proj(X)
    V = self.v_proj(X)
...
def _fast_prepare_inputs_for_generation(
    ...
    **kwargs,
    ...
    """
    Prepares inputs for generation by handling past key values and attention masks.
...
def fix_prepare_inputs_for_generation(module) -> None:
    """
    Patches the prepare_inputs_for_generation method of a module.
...
def fast_swiglu_inference(
    ...
    """
    Performs fast inference using SwiGLU activation.

    Args:
            X (`torch.Tensor`): Input tensor.
            temp_gate (`torch.Tensor`, optional): Temporary tensor for gate projection.
            temp_up (`torch.Tensor`, optional): Temporary tensor for up projection.

    Returns:
            `torch.Tensor`: Output tensor after SwiGLU operation.
    """
...
def fast_rms_layernorm_inference(
    self,
    X: torch.Tensor,
    XX: Optional[torch.Tensor] = None,
    XX2: Optional[torch.Tensor] = None,
    ...
    Args:
            X (`torch.Tensor`): Input tensor.
            XX (`torch.Tensor`, optional): Temporary tensor.
            XX2 (`torch.Tensor`, optional): Temporary tensor.
            variance (`torch.Tensor`, optional): Variance tensor.
...
def fast_rms_layernorm_inference_gemma(
    self, X: torch.Tensor, out_weight: Optional[torch.Tensor] = None
) -> torch.Tensor:
    """
    Performs fast RMS layer normalization optimized for Gemma models.

    Args:
            X (`torch.Tensor`): Input tensor.
            out_weight (`torch.Tensor`, optional): Output weight tensor.
...
def fast_layernorm_compiled(layernorm, X: torch.Tensor) -> torch.Tensor:
    """
    Compiled version of fast layer normalization.

    Args:
            layernorm: LayerNorm module.
            X (`torch.Tensor`): Input tensor.
...
def LlamaAttention_fast_forward(
    ...
    **kwargs,
    ...
    """
    Fast forward pass for Llama attention layer.

    Args:
            hidden_states (`torch.Tensor`): Input hidden states.
            causal_mask (`Any`, optional): Causal attention mask.
    ...
    Returns:
            Tuple[`torch.Tensor`, Optional[`torch.Tensor`], Optional[Tuple[`torch.Tensor`]]]: Output tensors.
    """
...
def LlamaModel_fast_forward(
    ...
    **kwargs,
    ...
    """
    Fast forward pass for Llama model.

    Args:
            input_ids (`torch.LongTensor`): Input token IDs.
            causal_mask (`BlockDiagonalCausalMask`, optional): Causal mask.
            attention_mask (`torch.Tensor`, optional): Attention mask.
            position_ids (`torch.LongTensor`, optional): Position IDs.
            ...
            return_dict (`bool`, optional): Whether to return dict.
    ...
    for idx, decoder_layer in enumerate(self.layers):
        ...
        try:
            is_gradient_checkpointing_layer = isinstance(
...
def _LlamaModel_fast_forward_inference(
    attention_fast_forward_inference: Callable = LlamaAttention_fast_forward_inference,
    mlp_fast_forward_inference: Callable = fast_swiglu_inference,
) -> Callable:
    """
    Creates a custom fast forward inference function for Llama model.
    ...
    Returns:
            `Callable`: Custom inference function.
    """
...
def CausalLM_fast_forward(fast_forward_inference: Callable) -> Callable:
    """
    Creates a fast forward function for causal language model.

    Args:
            fast_forward_inference (`Callable`): Forward inference function.

    Returns:
            `Callable`: Fast forward function.
    """
...
def PeftModelForCausalLM_fast_forward(
    ...
    **kwargs,
    ...
    """
    Fast forward pass for PEFT causal language model.

    Args:
            input_ids (`torch.LongTensor`, optional): Input token IDs.
            causal_mask (`Any`, optional): Causal mask.
            attention_mask (`torch.Tensor`, optional): Attention mask.
            inputs_embeds (`torch.FloatTensor`, optional): Input embeddings.
            labels (`torch.LongTensor`, optional): Labels.
            output_attentions (`bool`, optional): Whether to output attentions.
            output_hidden_states (`bool`, optional): Whether to output hidden states.
            return_dict (`bool`, optional): Whether to return dict.
            task_ids (`Any`, optional): Task IDs.
...
class LlamaRotaryEmbedding(torch.nn.Module):
    """
    Implements rotary embeddings for Llama models.

    Args:
            dim (`int`, optional): Dimension of embeddings.
            max_position_embeddings (`int`): Maximum position embeddings length.
            base (`int`): Base value for embeddings.
            device (`str`, optional): Device to use.
            config (`Any`, optional): Model config.

    Methods:
            _set_cos_sin_cache: Sets cosine and sine cache.
    ...
    def __init__(
        ...
        config: Optional[
            Any
        ] = None,  # [TODO] Hack to pass in config - need to remove later
    ):
        super().__init__()
    ...
    def _set_cos_sin_cache(self, seq_len: int, device: str, dtype: torch.dtype) -> None:
        """
        Sets cosine and sine cache for rotary embeddings.
    ...
    def extend_rope_embedding(self, x: torch.Tensor, seq_len: int) -> None:
        """
        Extends rope embeddings to longer sequence lengths.
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    ...
    def _set_cos_sin_cache(self, seq_len: int, device: str, dtype: torch.dtype) -> None:
        """
        Sets cosine and sine cache with linear scaling.
...
class LlamaExtendedRotaryEmbedding(torch.nn.Module):
    """
    Extended rotary embeddings with scaling factors.
    ...
    Methods:
            _set_cos_sin_cache: Sets cosine and sine cache.
            apply_scaling: Applies scaling to frequencies.
            forward: Forward pass.
            get_cached: Gets cached embeddings.
            extend_rope_embedding: Extends rope embeddings.
    """
    ...
    def _set_cos_sin_cache(self, seq_len: int, device: str, dtype: torch.dtype) -> None:
        """
        Sets cosine and sine cache for extended embeddings.
    ...
    def apply_scaling(self, freqs: torch.Tensor) -> torch.Tensor:
        """
        Applies scaling to frequencies.
    ...
    def forward(
        ...
        """
        Forward pass for extended embeddings.
...
class LongRopeRotaryEmbedding(torch.nn.Module):
    """
    Long rope rotary embeddings for extended context.

    Args:
            dim (`int`, optional): Dimension of embeddings.
            max_position_embeddings (`int`): Maximum position embeddings length.
            original_max_position_embeddings (`int`): Original maximum position embeddings length.
            base (`int`): Base value for embeddings.
            short_factor (`float`, optional): Short sequence scaling factor.
            long_factor (`float`, optional): Long sequence scaling factor.
            device (`str`, optional): Device to use.
            config (`Any`, optional): Model config.
    ...
    def _set_cos_sin_cache(self, seq_len: int, device: str, dtype: torch.dtype) -> None:
        """
        Sets cosine and sine cache for long rope embeddings.
...
def unsloth_fast_generate(
    ...
    **kwargs,
    ...
    """
    Fast generation function for Unsloth models.
...
class FastLlamaModel:
    """
    Fast implementation of Llama model with optimizations.

    Methods:
            pre_patch: Pre-patches model components.
            from_pretrained: Loads pretrained model.
            post_patch: Post-patches model components.
            get_peft_model: Gets PEFT model.
            patch_peft_model: Patches PEFT model.
            for_inference: Sets model for inference.
            for_training: Sets model for training.
    """

    @staticmethod
    def pre_patch() -> None:
        """
        Pre-patches model components for optimization.
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Loads pretrained model with optimizations.
        ...
        Args:
                ...
                rope_scaling (`Dict[str, Any]`, optional): Rope scaling config.
                fix_tokenizer (`bool`): Whether to fix tokenizer.
                model_patcher (`Any`, optional): Model patcher.
                tokenizer_name (`str`, optional): Tokenizer name.
                trust_remote_code (`bool`): Whether to trust remote code.
                fast_inference (`bool`): Whether to use fast inference.
                gpu_memory_utilization (`float`): GPU memory utilization.
                float8_kv_cache (`bool`): Whether to use 8-bit KV cache.
                random_state (`int`): Random seed.
                max_lora_rank (`int`): Maximum LoRA rank.
                disable_log_stats (`bool`): Whether to disable logging stats.
                **kwargs: Additional keyword arguments.
    ...
    def post_patch(model, tokenizer) -> Tuple[Any, Any]:
        """
        Post-patches model components after loading.
    ...
    def get_peft_model(
        ...
        **kwargs,
        ...
        """
        Gets PEFT model with optimizations.
        ...
        Args:
                ...
                bias (`str`): Bias type.
                layers_to_transform (`List[int]`, optional): Layers to transform.
                layers_pattern (`str`, optional): Layers pattern.
                use_gradient_checkpointing (`bool`): Whether to use gradient checkpointing.
                random_state (`int`): Random seed.
                max_seq_length (`int`): Maximum sequence length.
                use_rslora (`bool`): Whether to use RSLoRA.
                modules_to_save (`List[str]`, optional): Modules to save.
                init_lora_weights (`Union[bool, str]`): How to initialize LoRA weights.
                loftq_config (`Dict[str, Any]`): LoftQ config.
                temporary_location (`str`): Temporary location for buffers.
                **kwargs: Additional keyword arguments.
            ...
            if check_all:
                ...
                if "embed_tokens" in new_target_modules:
                    ...
                    model.get_input_embeddings().modules_to_save.default.to(
                        device="cuda", dtype=new_dtype, non_blocking=True
                    )
    ...
    def patch_peft_model(
        ...
        """
        Patches PEFT model with optimizations.

        Args:
                model: PEFT model to patch.
                use_gradient_checkpointing (`bool`): Whether to use gradient checkpointing.
