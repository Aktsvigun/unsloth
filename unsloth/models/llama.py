...
import math
import functools
from typing import Optional, Tuple, List, Union, Any, T, Dict, Callable
from ._utils import *
...
def _fast_prepare_inputs_for_generation(self, input_ids: torch.LongTensor, attention_mask: Optional[torch.Tensor]=None, **kwargs,) -> Dict[str, Any]:
	"""
	Prepares inputs for generation by handling past_key_values and attention_mask for inference.
	...
	Args:
		...
		**kwargs:
			Additional arguments including past_key_values
...
def fix_prepare_inputs_for_generation(module) -> None:
	"""
	Patches the prepare_inputs_for_generation method of a module to use faster inference preparation.
...
def fast_swiglu_inference(self, X: torch.Tensor, temp_gate: Optional[torch.Tensor] = None, temp_up: Optional[torch.Tensor] = None) -> torch.Tensor:
	"""
	Performs fast inference for SwiGLU activation function.
	...
	Args:
		...
		X (`torch.Tensor`):
			Input tensor
		temp_gate (`Optional[torch.Tensor]`):
			Temporary buffer for gate projection
		temp_up (`Optional[torch.Tensor]`):
			Temporary buffer for up projection
...
def fast_rms_layernorm_inference(self, X: torch.Tensor, XX: Optional[torch.Tensor] = None, XX2: Optional[torch.Tensor] = None, variance: Optional[torch.Tensor] = None) -> torch.Tensor:
	"""
	Performs fast inference for RMS LayerNorm.
	...
	Args:
		...
		XX (`Optional[torch.Tensor]`):
			Temporary buffer 1
		XX2 (`Optional[torch.Tensor]`):
			Temporary buffer 2
		variance (`Optional[torch.Tensor]`):
			Temporary buffer for variance
...
def fast_rms_layernorm_inference_gemma(self, X: torch.Tensor, out_weight: Optional[torch.Tensor] = None) -> torch.Tensor:
	"""
	Gemma-specific fast inference for RMS LayerNorm.
	...
	Args:
		...
		X (`torch.Tensor`):
			Input tensor
		out_weight (`Optional[torch.Tensor]`):
			Temporary buffer for output weights
...
def fast_layernorm_compiled(layernorm, X: torch.Tensor) -> torch.Tensor:
	"""
	Compiled version of LayerNorm for faster inference.
...
def LlamaAttention_fast_forward(
	...
	*args, **kwargs,
	...
	"""
	Fast forward pass for Llama attention with support for various attention implementations.
...
def _LlamaModel_fast_forward_inference(attention_fast_forward_inference: Callable=LlamaAttention_fast_forward_inference, mlp_fast_forward_inference: Callable=fast_swiglu_inference) -> Callable:
	"""
	Creates a custom fast forward function for Llama model inference.
	
	Args:
		attention_fast_forward_inference (`Callable`):
			Custom attention forward function
		mlp_fast_forward_inference (`Callable`):
			Custom MLP forward function
	
	Returns:
		`Callable`: Custom forward function
	"""
...
def CausalLM_fast_forward(fast_forward_inference: Callable) -> Callable:
	"""
	Creates a fast forward function for causal language modeling.
	
	Args:
		fast_forward_inference (`Callable`):
			Custom model forward function
...
def PeftModelForCausalLM_fast_forward(
	...
	**kwargs,
	...
	"""
	Fast forward pass for PEFT causal LM model.
...
class LlamaRotaryEmbedding(torch.nn.Module):
	"""
	Rotary positional embeddings for Llama models with dynamic cache growth.
	...
	Methods:
		...
		extend_rope_embedding: Extends cache dynamically
	"""
	# Fixes https://github.com/huggingface/transformers/pull/28837
	# https://github.com/microsoft/DeepSpeed/issues/4932
	# The precision of RoPE buffers is not correct, so we cast to int64.
	def __init__(self, dim: Optional[int] = None, max_position_embeddings: int=2048, base: float=10000, device: Optional[str]=None,
	...
	def _set_cos_sin_cache(self, seq_len: int, device: str, dtype: torch.dtype) -> None:
		"""
		Initializes cosine/sine cache for rotary embeddings.
	...
	def get_cached(self, seq_len: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
		"""
		Gets cached cosine/sine embeddings.
	...
	def extend_rope_embedding(self, x: torch.Tensor, seq_len: int) -> None:
		"""
		Extends rotary embeddings cache dynamically.
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
	...
	# https://github.com/microsoft/DeepSpeed/issues/4932
	# The precision of RoPE buffers is not correct, so we cast to int64.
	def __init__(self, dim: Optional[int] = None, max_position_embeddings: int=2048, base: float=10000, device: Optional[str]=None, scaling_factor: float=1.0,
		config: Optional[Any] = None, # [TODO] Hack to pass in config - need to remove later
	):
...
class LlamaExtendedRotaryEmbedding(torch.nn.Module):
	...
	Methods:
		...
		forward: Computes embeddings
		get_cached: Gets cached embeddings
		extend_rope_embedding: Extends cache
	"""
	def __init__(self, dim: Optional[int] = None, max_position_embeddings: int=2048, base: float=10000, device: Optional[str]=None,
		config: Optional[Any] = None, # [TODO] Hack to pass in config - need to remove later
	):
	...
	def get_cached(self, seq_len: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
		"""
		Gets cached extended embeddings.
	...
	def extend_rope_embedding(self, x: torch.Tensor, seq_len: int) -> None:
		"""
		Extends extended rotary embeddings cache.
...
class LongRopeRotaryEmbedding(torch.nn.Module):
	"""
	Long-range rotary embeddings for Phi 3.5 with separate short/long factors.
	...
	Args:
		...
		base (`float`):
			Base for frequency calculation
		short_factor (`Optional[float]`):
			Scaling for short sequences
		long_factor (`Optional[float]`):
			Scaling for long sequences
		device (`Optional[str]`):
			Device to use
		config (`Optional[Any]`):
			Model config
	...
	def _set_cos_sin_cache(self, seq_len: int, device: str, dtype: torch.dtype) -> None:
		"""
		Initializes cosine/sine cache for long-range embeddings.
	...
	def forward(self, x: torch.Tensor, position_ids: Optional[torch.LongTensor]=None, seq_len: Optional[int]=None) -> Tuple[torch.Tensor, torch.Tensor]:
		"""
		Computes long-range rotary embeddings.
	...
	def get_cached(self, seq_len: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
		"""
		Gets cached long-range embeddings.
	...
	def extend_rope_embedding(self, x: torch.Tensor, seq_len: int) -> None:
		"""
		Extends long-range embeddings cache.
...
def unsloth_fast_generate(
	...
	**kwargs,
	...
	"""
	Fast generation function with inference optimizations.
...
class FastLlamaModel:
	"""
	Optimized implementation of Llama models with various speed improvements.
	
	Methods:
		pre_patch: Applies initial patches
		from_pretrained: Loads pretrained model
		post_patch: Applies post-load patches
		get_peft_model: Creates PEFT model
		patch_peft_model: Optimizes PEFT model
		for_inference: Configures for inference
		for_training: Configures for training
	"""

	@staticmethod
	def pre_patch() -> None:
		"""
		Applies initial patches to model classes.
		...
		**kwargs,
		...
		Args:
			...
			rope_scaling (`Optional[Dict[str, Any]]`):
				RoPE scaling config
			fix_tokenizer (`bool`):
				Whether to fix tokenizer
			model_patcher (`Optional[Any]`):
				Custom model patcher
	...
	def post_patch(model, tokenizer) -> Tuple[Any, Any]:
		"""
		Applies post-load patches to model and tokenizer.
	...
	def get_peft_model(
		...
		**kwargs,
		...
		"""
		Creates PEFT model with optimizations.
	...
	def patch_peft_model(
		...
		"""
		Optimizes PEFT model with custom forward passes.
