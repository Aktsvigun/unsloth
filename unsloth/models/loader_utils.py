...
def __get_model_name(
    ...
    """
    Internal function to determine the appropriate model name based on quantization settings and transformer version compatibility.

    Args:
            model_name (`str`):
                    The original model name to check.
            load_in_4bit (`bool`, defaults to `True`):
                    Whether to load the model in 4-bit precision.
            INT_TO_FLOAT_MAPPER (`dict[str, str]`, *optional*):
                    Mapping from integer to float model names.
            FLOAT_TO_INT_MAPPER (`dict[str, str]`, *optional*):
                    Mapping from float to integer model names.
            MAP_TO_UNSLOTH_16bit (`dict[str, str]`, *optional*):
                    Mapping to Unsloth 16-bit model names.

    Returns:
            `str` or `None`: The adjusted model name if a mapping exists, otherwise None.
    """
...
def _get_new_mapper() -> tuple[dict[str, str], dict[str, str], dict[str, str]]:
    """
    Fetches updated model name mappings from a remote source to support newer Unsloth versions.

    Returns:
            `tuple[dict, dict, dict]`: A tuple containing updated INT_TO_FLOAT_MAPPER, FLOAT_TO_INT_MAPPER, and MAP_TO_UNSLOTH_16bit mappings.
            If the remote fetch fails, returns empty dictionaries.
    """
    try:
        import requests
...
def get_model_name(model_name: str, load_in_4bit: bool = True) -> str:
    """
    Determines the appropriate model name based on quantization settings, version compatibility, and known bad mappings.
    ...
    Returns:
            `str`: The adjusted model name if a mapping exists, otherwise the original model name.
    """
