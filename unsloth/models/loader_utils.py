...
def __get_model_name(
	...
	"""
	Internal function to determine the appropriate model name based on quantization settings and available transformers version.
	
	Args:
		model_name (`str`): The original model name to check.
		load_in_4bit (`bool`, defaults to `True`): Whether to load the model in 4-bit precision.
		INT_TO_FLOAT_MAPPER (`dict[str, str]`, *optional*): Mapping from integer to float model names.
		FLOAT_TO_INT_MAPPER (`dict[str, str]`, *optional*): Mapping from float to integer model names.
		MAP_TO_UNSLOTH_16bit (`dict[str, str]`, *optional*): Mapping to Unsloth 16-bit model names.
	
	Returns:
		`Optional[str]`: The adjusted model name based on quantization requirements, or None if no adjustment is needed.
	"""
...
def _get_new_mapper() -> tuple[dict[str, str], dict[str, str], dict[str, str]]:
	"""
	Fetches updated model name mappings from the Unsloth GitHub repository.
	
	Returns:
		`tuple[dict[str, str], dict[str, str], dict[str, str]]`: A tuple containing updated INT_TO_FLOAT_MAPPER, FLOAT_TO_INT_MAPPER, and MAP_TO_UNSLOTH_16bit dictionaries.
	"""
	try:
		import requests
...
def get_model_name(model_name: str, load_in_4bit: bool = True) -> str:
	"""
	Determines the appropriate model name based on quantization settings, available transformers version, and model availability.
	...
	Returns:
		`str`: The adjusted model name based on quantization requirements and model availability, or the original model name if no adjustments are needed.
	"""
