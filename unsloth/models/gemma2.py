from typing import Optional, T, Tuple, Any
...
def fast_rms_layernorm_gemma2_compiled(layernorm, X: torch.Tensor, gemma: bool = True) -> torch.Tensor:
    old_dtype = X.dtype
    X = X.float()
...
class FastGemma2Model(FastLlamaModel):
    ...
    @staticmethod
    def post_patch(model: Gemma2ForCausalLM, tokenizer: PreTrainedTokenizer) -> tuple[Gemma2ForCausalLM, PreTrainedTokenizer]:
