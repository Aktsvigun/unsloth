...
class Fast_CrossEntropyLoss(torch.autograd.Function):
	...
	It's designed to be used as a drop-in replacement for PyTorch's CrossEntropyLoss but with performance optimizations.
	
	Features:
	- Optimized for large vocabularies (supports both small and large vocab sizes)
	- Supports logit softcapping (for Gemma 2)
	- Supports logit scaling (for Cohere)
	- Memory efficient implementation using Triton kernels
	
	Args:
		logits (torch.Tensor):
			Input tensor of shape (batch_size, vocab_size) containing unnormalized logits
		labels (torch.Tensor):
			Ground truth labels of shape (batch_size,) containing class indices (0 to vocab_size-1)
		logit_softcapping (float):
			Optional softcapping value for logits (used in Gemma 2)
		logit_scaling (float):
			Optional scaling factor for logits (used in Cohere models)
	"""
	@staticmethod
	def forward(ctx, logits: torch.Tensor, labels: torch.Tensor, logit_softcapping : float = 0, logit_scaling : float = 0) -> torch.Tensor:
		...
		This method calculates the cross entropy loss between the input logits and the ground truth labels.
		It handles both small and large vocabularies efficiently using different Triton kernels.
		...
		Returns:
			torch.Tensor: Computed loss values of shape (batch_size,)
		"""
		n_rows : int
		vocab_size : int
	...
	def backward(ctx, dlosses: torch.Tensor) -> tuple[torch.Tensor, None, None, None]:
		...
		It uses the saved values from the forward pass to compute the gradients efficiently.
		...
		Returns:
			tuple[torch.Tensor, None, None, None]:
				- Gradients of the loss with respect to the input logits
				- None for labels (no gradient needed)
				- None for logit_softcapping (no gradient needed)
				- None for logit_scaling (no gradient needed)
		"""
...
def patch_loss_functions(torch_compile: bool = True) -> None:
	"""
	Patch the cross entropy loss functions in the transformers library to use the fast implementation.
	
	This function replaces the standard cross entropy loss functions in the transformers library
	with the optimized version implemented in this module.
