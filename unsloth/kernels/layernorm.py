...
class Fast_Layernorm(torch.autograd.Function):
    @staticmethod
    def forward(ctx: torch.autograd.function.FunctionCtx, X: torch.Tensor, W: torch.Tensor, b: torch.Tensor, eps: float) -> torch.Tensor:
        shape = X.shape
        dim = shape[-1]
        X = X.view(-1, dim)
    ...
    @staticmethod
    def backward(ctx: torch.autograd.function.FunctionCtx, dY: torch.Tensor) -> tuple[torch.Tensor, None, None, None, None]:
        shape = dY.shape
        dim = shape[-1]
        dY = dY.view(-1, dim)
