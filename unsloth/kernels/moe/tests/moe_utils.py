...
def rebind_experts_to_shared_buffer(
	...
	Returns:
		`tuple[torch.Tensor, torch.Tensor, torch.Tensor]`: A tuple containing the shared buffers for the up projection, gate projection, and down projection weights.
	"""
...
def get_expert_metadata(model_id: str) -> dict:
	"""
	Fetches the metadata of the experts in a MoE block.
	
	Args:
		model_id (`str`):
			The ID of the model to fetch the expert metadata from.
	
	Returns:
		`dict`: A dictionary containing the metadata of the experts.
	"""
	api = HfApi()
...
def clone_experts(
	...
	"""
	Clones the expert weights of a MoE block.
	...
	Returns:
		`tuple[torch.Tensor, torch.Tensor, torch.Tensor]`: A tuple containing the cloned gate projection, up projection, and down projection weights.
	"""
...
class ForwardResult:
	"""
	A dataclass that holds the results of a forward pass through a MoE block.
	
	Args:
		output (`torch.Tensor`):
			The output of the MoE block.
		router_logits (`torch.Tensor`):
			The logits used for routing the input to the experts.
		X (`torch.Tensor`):
...
class BackwardResult:
	"""
	A dataclass that holds the results of a backward pass through a MoE block.
	
	Args:
		X_grad (`torch.Tensor`):
			The gradient of the input to the MoE block.
...
def check_down_proj_grad(
	...
	"""
	Checks the gradient of the down projection weights.
	
	Args:
		moe_block (`Qwen3MoeSparseMoeBlock`):
			The MoE block whose down projection gradient is to be checked.
		grouped_gemm_block (`Qwen3MoeGroupedGEMMBlock`):
			The grouped GEMM block whose down projection gradient is to be checked.
		atol (`float`):
			The absolute tolerance for the gradient check.
		rtol (`float`):
			The relative tolerance for the gradient check.
	"""
	for i, expert in enumerate(moe_block.experts):
...
def check_gate_up_proj_grad(
	...
	"""
	Checks the gradient of the gate and up projection weights.
	
	Args:
		moe_block (`Qwen3MoeSparseMoeBlock`):
			The MoE block whose gate and up projection gradients are to be checked.
		grouped_gemm_block (`Qwen3MoeGroupedGEMMBlock`):
			The grouped GEMM block whose gate and up projection gradients are to be checked.
		atol (`float`):
			The absolute tolerance for the gradient check.
		rtol (`float`):
			The relative tolerance for the gradient check.
...
def check_gate_grad(
	...
	"""
	Checks the gradient of the gate weights.
	
	Args:
		moe_block (`Qwen3MoeSparseMoeBlock`):
			The MoE block whose gate gradient is to be checked.
		grouped_gemm_block (`Qwen3MoeGroupedGEMMBlock`):
			The grouped GEMM block whose gate gradient is to be checked.
		atol (`float`):
			The absolute tolerance for the gradient check.
		rtol (`float`):
			The relative tolerance for the gradient check.
...
def check_wgrad(
	...
	"""
	Checks the gradients of the weights.
	
	Args:
		moe_block (`Qwen3MoeSparseMoeBlock`):
			The MoE block whose weights' gradients are to be checked.
		grouped_gemm_block (`Qwen3MoeGroupedGEMMBlock`):
			The grouped GEMM block whose weights' gradients are to be checked.
		atol (`float`):
			The absolute tolerance for the gradient check.
		rtol (`float`):
			The relative tolerance for the gradient check.
...
def check_tensor_allclose(
	...
	Args:
		...
		atol (`float`):
			The absolute tolerance for the tensor comparison.
		rtol (`float`):
			The relative tolerance for the tensor comparison.
		name (`str`):
			The name of the tensor for error reporting.
		verbose (`bool`, optional):
			Whether to print the difference between the tensors. Defaults to `False`.
	"""
...
def check_expert_grads(
	...
	"""
	Checks the gradients of the experts.
	
	Args:
		ref_result (`BackwardResult`):
			The reference backward result.
		test_result (`BackwardResult`):
			The backward result to be tested.
		atol (`float`):
			The absolute tolerance for the gradient check.
		rtol (`float`):
			The relative tolerance for the gradient check.
		verbose (`bool`, optional):
			Whether to print the difference between the gradients. Defaults to `False`.
	"""
...
def check_fwd(
	...
	"""
	Checks the forward pass.
	
	Args:
		ref_result (`ForwardResult`):
			The reference forward result.
		test_result (`ForwardResult`):
			The forward result to be tested.
		atol (`float`):
			The absolute tolerance for the forward pass check.
		rtol (`float`):
			The relative tolerance for the forward pass check.
		verbose (`bool`, optional):
			Whether to print the difference between the forward results. Defaults to `False`.
	"""
...
def check_grouped_gemm_results(
	...
	"""
	Checks the results of the grouped GEMM operation.
	
	Args:
		grouped_result (`GroupedGEMMResult`):
			The reference grouped GEMM result.
		fused_result (`GroupedGEMMResult`):
			The grouped GEMM result to be tested.
		permute_y (`bool`):
			Whether the output of the grouped GEMM operation is permuted.
		atol (`float`):
			The absolute tolerance for the grouped GEMM result check.
		rtol (`float`):
			The relative tolerance for the grouped GEMM result check.
		verbose (`bool`, optional):
			Whether to print the difference between the grouped GEMM results. Defaults to `False`.
	"""
...
def run_forward(model: nn.Module, X: torch.Tensor, is_grouped_gemm: bool = False) -> ForwardResult:
	"""
	Runs the forward pass through a model.
	
	Args:
		model (`nn.Module`):
			The model to run the forward pass through.
		X (`torch.Tensor`):
			The input to the model.
		is_grouped_gemm (`bool`, optional):
			Whether the model uses grouped GEMM. Defaults to `False`.
...
def run_backward(
	...
	"""
	Runs the backward pass through a model.
	
	Args:
		model (`nn.Module`):
			The model to run the backward pass through.
		grad_output (`torch.Tensor`):
			The gradient of the output of the model.
		output (`torch.Tensor`):
...
class Qwen3MoeFusedGroupedGEMMBlock(Qwen3MoeGroupedGEMMBlock):
	...
	def from_hf(
		...
		"""
		Creates a `Qwen3MoeFusedGroupedGEMMBlock` from a Hugging Face model.
		
		Args:
			moe_block (`Qwen3MoeSparseMoeBlock`):
				The MoE block to create the `Qwen3MoeFusedGroupedGEMMBlock` from.
			...
			autotune (`bool`, optional):
				Whether to autotune the kernel configuration. Defaults to `True`.
			kernel_config_fwd (`KernelConfigForward`, optional):
				The kernel configuration for the forward pass. Defaults to `None`.
			kernel_config_bwd_dW (`KernelConfigBackward_dW`, optional):
				The kernel configuration for the backward pass with respect to the weights. Defaults to `None`.
			kernel_config_bwd_dX (`KernelConfigBackward_dX`, optional):
				The kernel configuration for the backward pass with respect to the input. Defaults to `None`.
	...
	def forward(self, hidden_states: torch.Tensor, debug: bool = False) -> torch.Tensor:
		"""
		Runs the forward pass through the `Qwen3MoeFusedGroupedGEMMBlock`.
