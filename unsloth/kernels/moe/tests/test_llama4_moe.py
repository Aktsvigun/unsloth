...
def annotated_context(prelude: str, epilogue: str="Passed!", char: str="-", num_chars: int=80) -> None:
	...
	Args:
		...
		epilogue (`str`):
			Text to print after exiting the context.
		char (`str`):
			Character to use for the border.
		num_chars (`int`):
			Number of characters to use for the border.
	"""
...
def get_text_config(model_id: str) -> Llama4TextConfig:
	"""
	Get the text config from a Llama4 model.
	
	Args:
		model_id (`str`):
			The model id to load the config from.
	
	Returns:
		`Llama4TextConfig`: The text config of the model.
	"""
...
def prep_triton_kernel_traits(autotune: bool) -> tuple[Optional[KernelConfigForward], Optional[KernelConfigBackward_dW], Optional[KernelConfigBackward_dX]]:
	"""
	Prepare the kernel traits for the triton kernels.
	
	Args:
		autotune (`bool`):
			Whether to use autotuned kernels.
	
	Returns:
		`tuple[Optional[KernelConfigForward], Optional[KernelConfigBackward_dW], Optional[KernelConfigBackward_dX]]`:
			The kernel configs for forward, backward dW and backward dX.
	"""
	if not autotune:
...
def _check_diff(
	...
	Args:
		...
		precision (`str`):
			The precision to use when printing the difference.
		verbose (`bool`):
			Whether to print the difference.
		msg (`str`):
			The message to print before the difference.
	"""
	t2 = t2.view_as(t1)
...
def run_backwards(y: torch.Tensor, grad_output: torch.Tensor, module: torch.nn.Module) -> None:
	...
	Args:
		...
		module (`torch.nn.Module`):
			The module to run the backward pass on.
	"""
...
def model_config() -> Llama4TextConfig:
	"""
	Fixture that returns the model config for the Llama4 model.
	
	Returns:
		`Llama4TextConfig`: The model config.
	"""
...
def test_llama4_ref(
	...
	"""
	Test the Llama4 reference implementation against the grouped gemm implementation.
