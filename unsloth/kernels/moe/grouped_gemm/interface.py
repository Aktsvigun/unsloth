...
def supports_tma() -> bool:
	"""
	Check if the current CUDA device supports TMA (Tensor Memory Acceleration).
	
	Returns:
		`bool`: True if TMA is supported, False otherwise.
	"""
...
def get_per_device_per_stream_alloc_fn(device: torch.device) -> Callable[[int, int, int], torch.Tensor]:
	"""
	Get a function that allocates memory on a per-device and per-stream basis.
	...
	Returns:
		`Callable[[int, int, int], torch.Tensor]`: A function that allocates memory on the specified device.
	"""
...
def log_kernel_info(
	...
	Args:
		compiled_kernel (`triton.compiler.CompiledKernel`):
			The compiled kernel to log information about.
		best_config (`triton.Config`, *optional*):
			The best configuration for the kernel, if available.
	"""
...
class GroupedGemm(torch.autograd.Function):
	"""
	A class for performing grouped GEMM operations, which are commonly used in MoE (Mixture of Experts) MLPs.
	
	This class provides methods for forward and backward passes of grouped GEMM operations, with support for various optimizations and fusions specific to MoE.
	"""
	@staticmethod
	def forward(
		ctx,
		...
		"""
		Perform the forward pass of the grouped GEMM operation.
		...
		Args:
			...
			W (`torch.Tensor`):
				The weight tensor.
			m_sizes (`torch.Tensor`):
				The sizes of the tokens assigned to each expert.
			topk (`int`):
				The number of experts chosen per token.
			gather_indices (`torch.Tensor`):
				The indices of tokens assigned to each expert.
			permute_x (`bool`):
				Whether to permute the input tensor X.
			permute_y (`bool`):
				Whether to permute the output tensor Y.
			topk_weights (`Optional[torch.Tensor]`):
				The weights to multiply the routed output by.
			fuse_mul_post (`bool`):
				Whether to fuse the multiplication of the routed output with topk_weights.
			kernel_config_fwd (`Optional[KernelConfigForward]`):
				The configuration for the forward kernel.
			kernel_config_bwd_dX (`Optional[KernelConfigBackward_dX]`):
				The configuration for the backward kernel for dX.
			kernel_config_bwd_dW (`Optional[KernelConfigBackward_dW]`):
				The configuration for the backward kernel for dW.
			autotune (`bool`):
				Whether to autotune the kernel.
			dX_only (`bool`):
				Whether to compute only dX.
			dW_only (`bool`):
				Whether to compute only dW.
	...
	def backward(ctx, dY: torch.Tensor) -> tuple[Optional[torch.Tensor], Optional[torch.Tensor], None, None, None, None, None, None, None, None, None, None]:
		"""
		Perform the backward pass of the grouped GEMM operation.
		...
		Returns:
			`tuple[Optional[torch.Tensor], Optional[torch.Tensor], None, None, None, None, None, None, None, None, None, None]`: The gradients of the input tensors.
		"""
