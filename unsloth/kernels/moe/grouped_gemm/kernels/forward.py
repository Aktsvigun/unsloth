...
def _grouped_gemm_forward_kernel(
	...
	Args:
		...
		gather_indices_ptr (tl.tensor): Pointer to tensor containing indices for permutation.
		topk_weights_ptr (tl.tensor): Pointer to tensor containing weights for fused multiplication.
		NUM_EXPERTS (tl.constexpr): Total number of experts.
		NUM_TOKENS (tl.constexpr): Total number of tokens.
		TOPK (tl.constexpr): Number of experts to select per token.
		N (tl.constexpr): Output dimension of the matrix multiplication.
		K (tl.constexpr): Input dimension of the matrix multiplication.
		NUM_SMS (tl.constexpr): Number of streaming multiprocessors to use.
		...
		FLATTEN (tl.constexpr): Whether to flatten the computation across experts.
	
	Returns:
		None: The result is stored in y_ptr.
	"""
