...
def _grouped_gemm_dX_kernel(
	...
	"""
	Triton kernel for backward pass of grouped GEMM operation to compute gradient with respect to input X.
	
	Args:
		dY_ptr (torch.Tensor): Gradient of the loss with respect to the output of shape [M_total, N]
		w_ptr (torch.Tensor): Weights tensor of shape [E, N, K]
		dX_ptr (torch.Tensor): Output gradient tensor of shape [M_total, K]
		gather_indices_ptr (torch.Tensor): Indices for gathering tokens
		m_sizes_ptr (torch.Tensor): Sizes of each expert's input tokens
		NUM_EXPERTS (tl.constexpr): Number of experts
		NUM_TOKENS (tl.constexpr): Number of tokens
		TOPK (tl.constexpr): Top-k value for expert selection
		...
		BLOCK_SIZE_K (tl.constexpr): Block size along K dimension
		PERMUTE_X (tl.constexpr): Whether to permute X during computation
		PERMUTE_Y (tl.constexpr): Whether to permute Y during computation
		USE_TMA_LOAD_W (tl.constexpr): Whether to use TMA for loading weights
		USE_TMA_LOAD_dY (tl.constexpr): Whether to use TMA for loading dY
		USE_TMA_STORE (tl.constexpr): Whether to use TMA for storing results
...
def _grouped_gemm_dW_kernel(
	...
	"""
	Triton kernel for backward pass of grouped GEMM operation to compute gradient with respect to weights W.
	
	Args:
		x_ptr (torch.Tensor): Input tensor of shape [TOTAL_TOKENS, K]
		dY_ptr (torch.Tensor): Gradient of the loss with respect to output of shape [TOTAL_TOKENS, N]
		dW_ptr (torch.Tensor): Output gradient tensor of shape [E, N, K]
		m_sizes_ptr (torch.Tensor): Sizes of each expert's input tokens
		gather_indices_ptr (torch.Tensor): Indices for gathering tokens
		NUM_TOKENS (tl.constexpr): Number of tokens
		TOPK (tl.constexpr): Top-k value for expert selection
		...
		PERMUTE_Y (tl.constexpr): Whether to permute Y during computation
		USE_TMA_LOAD_dY (tl.constexpr): Whether to use TMA for loading dY
		USE_TMA_LOAD_X (tl.constexpr): Whether to use TMA for loading input X
		USE_TMA_STORE (tl.constexpr): Whether to use TMA for storing results
		FLATTEN (tl.constexpr): Whether to flatten computation across experts
