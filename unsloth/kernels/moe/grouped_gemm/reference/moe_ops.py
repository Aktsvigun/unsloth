...
def unpermute(X: torch.Tensor, gather_indices: torch.Tensor) -> torch.Tensor:
	"""
	Reverses the permutation applied by the `permute` function, restoring the original order of tokens.
	
	Args:
		X (`torch.Tensor`):
			The permuted tensor of shape `[total_tokens, hidden_dim]` where `total_tokens = num_tokens * topk`.
		gather_indices (`torch.Tensor`):
			The indices used to permute the original tensor, typically generated by `permute`.
	
	Returns:
		`torch.Tensor`: The unpermuted tensor with shape matching the original input to `permute`.
	"""
