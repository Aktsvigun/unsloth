...
class Llama4MoeResult:
	...
	Args:
		...
		hidden_states_after_weight_merge (`torch.Tensor`):
			Hidden states after merging with expert weights.
		first_gemm (`torch.Tensor`):
			Output of the first grouped GEMM operation.
		intermediate (`torch.Tensor`):
			Intermediate activation after applying activation function.
		...
		final_out (`torch.Tensor`):
			Final output after combining expert outputs.
...
class Llama4GroupedGemmTextMoe(Llama4TextMoe):
	"""
	Implements the Llama4 Mixture of Experts (MoE) block using grouped GEMM operations.
	
	This class extends the base Llama4TextMoe to utilize grouped GEMM for efficient expert computation.
	...
	def copy_weights(self, other: Llama4TextMoe) -> Llama4GroupedGemmTextMoe:
		"""
		Copies weights from another Llama4TextMoe instance into this instance.
		
		Args:
			other (`Llama4TextMoe`):
				The source model to copy weights from.
		
		Returns:
			`Llama4GroupedGemmTextMoe`: The instance with copied weights.
		"""
	...
	def check_weights(self, other: Llama4TextMoe) -> None:
		"""
		Checks if weights match another Llama4TextMoe instance.
	...
	def act_and_mul(self, x: torch.Tensor) -> torch.Tensor:
		"""
		Applies activation function and multiplies gate and up projections.
		
		Args:
			x (`torch.Tensor`):
				Input tensor containing gate and up projections.
		
		Returns:
			`torch.Tensor`: Activated and multiplied output.
		"""
	...
	def get_token_counts_and_gather_indices(
		...
		"""
		Generates token counts per expert and gather indices for expert computation.
		...
		Returns:
			`Tuple[torch.Tensor, torch.Tensor]`:
				...
				- gather_indices: Indices to gather tokens for each expert
		"""
...
class Llama4TritonTextMoe(Llama4GroupedGemmTextMoe):
	"""
	Implements the Llama4 Mixture of Experts (MoE) block using Triton-based grouped GEMM operations.
	
	This class extends Llama4GroupedGemmTextMoe to utilize Triton for optimized expert computation.
	...
	Args:
		...
		permute_x (`bool`, *optional*, defaults to `False`):
			Whether to permute input tensor.
		permute_y (`bool`, *optional*, defaults to `True`):
			Whether to permute output tensor.
		...
		kernel_config_fwd (`KernelConfigForward`, *optional*):
			Configuration for forward pass kernel.
		kernel_config_bwd_dW (`KernelConfigBackward_dW`, *optional*):
			Configuration for backward pass kernel for weights.
		kernel_config_bwd_dX (`KernelConfigBackward_dX`, *optional*):
			Configuration for backward pass kernel for inputs.
	...
	def copy_weights(self, other: Llama4TextMoe) -> Llama4TritonTextMoe:
		...
		Returns:
			`Llama4TritonTextMoe`: The instance with copied weights.
		"""
		for name, param_to_copy in other.named_parameters():
			if self.verbose:
				print(f"Copying {name} with shape {param_to_copy.shape}")
