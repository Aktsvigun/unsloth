...
class GroupedGEMMResult:
	"""
	A dataclass that holds the intermediate and final results of the grouped GEMM (General Matrix Multiply) operations in the Qwen3 MoE (Mixture of Experts) block. This class is used to store various tensors generated during the forward pass for debugging and analysis purposes.
	"""
...
class Qwen3MoeGroupedGEMMBlock(torch.nn.Module):
	"""
	A reference implementation of the Qwen3 MoE block using grouped GEMM operations. This class provides a PyTorch-native implementation for the Mixture of Experts (MoE) layer in the Qwen3 model, using grouped matrix multiplications for efficiency.
	"""
	def __init__(
		self,
	...
	def extract_hf_weights(moe_block: Qwen3MoeSparseMoeBlock) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
		"""
		Extracts and organizes the weights from a Hugging Face Qwen3 MoE block into a format suitable for the grouped GEMM implementation. This method stacks the gate, gate projection, and down projection weights from all experts into tensors that can be used in the grouped GEMM operations.
		"""
	...
	def from_hf(cls, moe_block: Qwen3MoeSparseMoeBlock) -> Qwen3MoeGroupedGEMMBlock:
		"""
		Constructs a Qwen3MoeGroupedGEMMBlock from a Hugging Face Qwen3 MoE block. This class method extracts the necessary weights and configuration from the Hugging Face model and initializes a new instance of the grouped GEMM implementation.
		"""
		config: Qwen3MoeConfig = moe_block.experts[0].config
	...
	def check_weights(self, moe_block: Qwen3MoeSparseMoeBlock) -> None:
		"""
		Verifies that the weights in this grouped GEMM implementation match the weights from the original Hugging Face Qwen3 MoE block. This method performs assertions to ensure that the gate and up projection weights match the expected values from the Hugging Face implementation.
		"""
	...
	def act_and_mul(self, x: torch.Tensor) -> torch.Tensor:
		"""
		Applies the activation function and performs the element-wise multiplication between the gate and up projection results. This method implements the activation and multiplication step in the MoE block, which is a key part of the expert computation.
		"""
	...
	def run_router(self, hidden_states: torch.Tensor) -> torch.Tensor:
		"""
		Runs the routing mechanism to determine which experts should process each token. This method computes the router logits, applies softmax to get routing weights, selects the top-k experts for each token, and returns these routing decisions along with the router logits.
		"""
	...
	def get_token_counts_and_gather_indices(
		...
		"""
		Computes the number of tokens assigned to each expert and generates indices for gathering tokens in expert order. This method creates auxiliary data structures needed for efficient grouped GEMM operations by counting how many tokens are assigned to each expert and generating indices to reorder tokens for expert processing.
		"""
...
class Qwen3MoeFusedGroupedGEMMBlock(Qwen3MoeGroupedGEMMBlock):
	"""
	An optimized implementation of the Qwen3 MoE block using fused grouped GEMM operations. This class extends the basic grouped GEMM implementation with additional optimizations for performance, including kernel autotuning and configurable permutation strategies.
	"""
	def __init__(
		self,
		config: Qwen3MoeConfig,
	...
	def from_hf(
		...
		"""
		Constructs a Qwen3MoeFusedGroupedGEMMBlock from a Hugging Face Qwen3 MoE block with additional configuration options. This class method creates a new instance of the fused grouped GEMM implementation using weights from the Hugging Face model and allows for customization of permutation strategies and kernel configurations.
		"""
		config: Qwen3MoeConfig = moe_block.experts[0].config
	...
	def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
		"""
		Performs the forward pass through the fused grouped GEMM MoE block. This method implements the complete processing pipeline including routing, expert computation using optimized grouped GEMM operations, and result aggregation, with options to control permutation strategies and kernel configurations for performance optimization.
		"""
		batch_size, sequence_length, hidden_dim = hidden_states.shape
		num_tokens = batch_size * sequence_length
		total_tokens = num_tokens * self.top_k
