...
class Qwen3MoeFusedGroupedGEMMBlock(Qwen3MoeGroupedGEMMBlock):
	"""
	A reference implementation of the Mixture of Experts (MoE) block using grouped GEMM operations. This class extends Qwen3MoeGroupedGEMMBlock and provides additional functionality for permutation, autotuning, and kernel configuration.
	
	Args:
		config (`Qwen3MoeConfig`): Configuration object for the MoE block.
		gate (`torch.Tensor`): Gate weights for the MoE block.
		gate_up_proj (`torch.Tensor`): Gate and up projection weights.
		down_proj (`torch.Tensor`): Down projection weights.
		permute_x (`bool`, optional): Whether to permute input tensor X. Defaults to True.
		permute_y (`bool`, optional): Whether to permute output tensor Y. Defaults to True.
		autotune (`bool`, optional): Whether to enable autotuning for kernel selection. Defaults to True.
		kernel_config_fwd (`KernelConfigForward`, optional): Configuration for forward kernel. Required if autotune is False.
		kernel_config_bwd_dW (`KernelConfigBackward_dW`, optional): Configuration for backward gradient w.r.t weights. Required if autotune is False.
		kernel_config_bwd_dX (`KernelConfigBackward_dX`, optional): Configuration for backward gradient w.r.t input. Required if autotune is False.
		dW_only (`bool`, optional): Whether to compute only gradient w.r.t weights. Defaults to False.
		dX_only (`bool`, optional): Whether to compute only gradient w.r.t input. Defaults to False.
	"""
	def __init__(
		self,
	...
	def from_hf(
		...
		"""
		Constructs a Qwen3MoeFusedGroupedGEMMBlock from a HuggingFace MoE block.
		
		Args:
			moe_block (`Qwen3MoeSparseMoeBlock`): The HuggingFace MoE block to convert.
			permute_x (`bool`, optional): Whether to permute input tensor X. Defaults to True.
			permute_y (`bool`, optional): Whether to permute output tensor Y. Defaults to True.
			autotune (`bool`, optional): Whether to enable autotuning for kernel selection. Defaults to True.
			kernel_config_fwd (`KernelConfigForward`, optional): Configuration for forward kernel. Required if autotune is False.
		...
		Returns:
			`Qwen3MoeFusedGroupedGEMMBlock`: Constructed instance.
		"""
	...
	def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
		"""
		Performs forward pass through the MoE block.
		
		Args:
			hidden_states (`torch.Tensor` of shape `(batch_size, sequence_length, hidden_dim)`):
				Input tensor containing hidden states.
		
		Returns:
			`torch.Tensor`: Output tensor after applying MoE operations.
