...
def calculate_settings(n: int) -> (
    ...
    """
    Calculates optimal Triton kernel settings for a given tensor size.

    Args:
            n (`int`):
                    Size of the tensor dimension to process.

    Returns:
            `tuple(int, int)`:
                    A tuple containing (BLOCK_SIZE, num_warps) where:
                    - BLOCK_SIZE is the next power of two greater than n (up to MAX_FUSED_SIZE)
                    - num_warps is the number of warps to use based on block size
...
def _get_tensor_stream(tensor: torch_Tensor) -> c_void_p:
    """
    Gets the CUDA stream pointer for a given tensor's device.

    Args:
            tensor (`torch.Tensor`):
                    The tensor whose device stream we want to get.

    Returns:
            `ctypes.c_void_p`:
                    Pointer to the CUDA stream associated with the tensor's device.
    """
...
def QUANT_STATE(W: torch.Tensor):
    """
    Retrieves the quantization state from a tensor if it exists.

    Args:
            W (`torch.Tensor`):
                    The tensor to check for quantization state.

    Returns:
            `Any`:
                    The quantization state if present (W.quant_state), otherwise None.
    """
...
def get_lora_parameters(
    ...
    torch.Tensor, Any, Optional[torch.Tensor], Optional[torch.Tensor], Optional[float]
]:
    """
    Extracts LoRA parameters from a projection layer.

    Args:
            proj (`nn.Module`):
                    The projection layer containing LoRA parameters.

    Returns:
            `tuple(torch.Tensor, Any, Optional[torch.Tensor], Optional[torch.Tensor], Optional[float])`:
                    A tuple containing:
                    - Base weight matrix
                    - Quantization state
                    - LoRA A matrix (if exists)
                    - LoRA B matrix (if exists)
                    - Scaling factor (if exists)
    """
...
def get_lora_parameters_bias(
    ...
    """
    Extracts LoRA parameters and bias from a projection layer.
    ...
    Returns:
            `tuple(torch.Tensor, Any, Optional[torch.Tensor], Optional[torch.Tensor], Optional[float], Optional[torch.Tensor])`:
                    A tuple containing:
                    - Base weight matrix
                    - Quantization state
                    - LoRA A matrix (if exists)
...
if HAS_CUDA_STREAM:
    ...
    def fast_dequantize(W, quant_state=None, out=None, use_global_buffer=False):
        ...
        if out is None:
            ...
            assert out.dtype == dtype
        out_absmax = torch_empty(
            n_elements_absmax, dtype=torch.float32, device=device, requires_grad=False
        )
...
def fast_linear_forward(
    proj,
    X: torch.Tensor,
    temp_lora: Optional[torch.Tensor] = None,
    out: Optional[torch.Tensor] = None,
) -> torch.Tensor:
    """
    Performs optimized linear forward pass with optional LoRA and quantization support.

    Args:
            proj (`nn.Module`):
                    The projection layer containing weights and LoRA parameters
            X (`torch.Tensor`):
                    Input tensor of shape (batch_size, seq_len, in_dim)
            temp_lora (`torch.Tensor`, *optional*):
                    Temporary buffer for LoRA computation
            out (`torch.Tensor`, *optional*):
                    Output tensor to store results

    Returns:
            `torch.Tensor`:
                    Output tensor of shape (batch_size, seq_len, out_dim) with applied linear transformation,
                    including LoRA and quantization if enabled
    """
...
def matmul_lora(
    ...
    """
    Performs matrix multiplication with optional LoRA adaptation.

    Args:
            X (`torch.Tensor`):
                    Input tensor of shape (batch_size, seq_len, in_dim) or (batch_size, in_dim)
            W (`torch.Tensor`):
                    Base weight matrix
            W_quant (`Any`):
                    Quantization state for W if quantized
    ...
    Returns:
            `torch.Tensor`:
                    Output tensor of shape (batch_size, seq_len, out_dim) or (batch_size, out_dim)
                    with applied matrix multiplication and LoRA adaptation if enabled
    """
    dtype = X.dtype
    W = fast_dequantize(W.t(), W_quant, use_global_buffer=True)
