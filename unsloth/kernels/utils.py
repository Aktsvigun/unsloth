...
def calculate_settings(n : int) -> (int, int,):
	...
	Returns:
		`tuple(int, int)`:
			...
			- num_warps is the number of warps to use based on the block size
...
def QUANT_STATE(W: torch.Tensor) -> Optional[Any]:
	...
	Returns:
		`Optional[Any]`:
			The quantization state if present, otherwise None.
	"""
...
def get_lora_parameters(proj) -> tuple[torch.Tensor, Optional[Any], Optional[torch.Tensor], Optional[torch.Tensor], Optional[float]]:
	...
	Returns:
		`tuple(torch.Tensor, Optional[Any], Optional[torch.Tensor], Optional[torch.Tensor], Optional[float])`:
			A tuple containing:
			- Base weight matrix
			- Quantization state (if any)
			- LoRA A matrix (if any)
			- LoRA B matrix (if any)
...
def fast_linear_forward(proj, X: torch.Tensor, temp_lora: Optional[torch.Tensor] = None, out: Optional[torch.Tensor] = None) -> torch.Tensor:
	"""
	Performs optimized linear forward pass with support for quantized weights and LoRA.
	...
	Args:
		...
		temp_lora (`torch.Tensor`, *optional*):
			Temporary buffer for LoRA computations
	...
	Returns:
		`torch.Tensor`:
			...
			including quantization handling and LoRA adjustments
	"""
...
def matmul_lora(X: torch.Tensor, W: torch.Tensor, W_quant: Optional[Any], A: Optional[torch.Tensor], B: Optional[torch.Tensor], s: Optional[float], out: Optional[torch.Tensor] = None) -> torch.Tensor:
	"""
	Performs matrix multiplication with support for quantized weights and LoRA.
	...
	Args:
		...
		W_quant (`Optional[Any]`):
			Quantization state for the weights
	...
	Returns:
		`torch.Tensor`:
			...
			depending on input shape, with applied matrix multiplication, quantization handling
			and LoRA adjustments
	"""
	dtype = X.dtype
	W = fast_dequantize(W.t(), W_quant, use_global_buffer = True)
