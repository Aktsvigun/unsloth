...
class SyntheticDataKit:
    """
    A class for generating synthetic data using a pretrained language model. This class manages the vLLM server for efficient text generation and provides methods for data chunking and QA generation preparation.
    """
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        Instantiate a SyntheticDataKit from a pretrained model.
        ...
        Args:
                ...
                float8_kv_cache (`bool`): Whether to use float8 precision for KV cache.
                conservativeness (`float`): Conservativeness factor for memory allocation.
                token (`str`, *optional*): Hugging Face API token for private models.
                **kwargs: Additional arguments passed to vLLM.

        Returns:
                `SyntheticDataKit`: An initialized instance.
        """
    ...
    def check_vllm_status() -> bool:
        """
        Check if the vLLM server is running by querying its metrics endpoint.
    ...
    def cleanup(self) -> None:
        """
        Gracefully shut down the vLLM server and free GPU resources.
        """
    ...
    def __enter__(self) -> SyntheticDataKit:
        """
        Context manager entry point that returns the instance itself.
        """
        return self
    ...
    def __del__(self) -> None:
        """
        Destructor that ensures cleanup when instance is deleted.
        """
        self.cleanup()

    def chunk_data(self, filename: str = None) -> list[str]:
        """
        Split input text data into chunks suitable for processing by the model.

        Args:
                filename (`str`): Path to input text file.

        Returns:
                `list[str]`: List of paths to chunked output files.
        """
    ...
    def prepare_qa_generation(
        ...
        """
        Prepare configuration for synthetic QA generation.

        Args:
                output_folder (`str`): Directory for output files.
                max_generation_tokens (`int`): Maximum tokens for generation.
                temperature (`float`): Sampling temperature for generation.
                top_p (`float`): Top-p sampling parameter.
                overlap (`int`): Token overlap between chunks.
                default_num_pairs (`int`): Default number of QA pairs per chunk.
                cleanup_threshold (`float`): Threshold for response quality filtering.
                cleanup_batch_size (`int`): Batch size for cleanup process.
                cleanup_temperature (`float`): Temperature for cleanup sampling.
        """
        ...
        with open("synthetic_data_kit_config.yaml", "w") as f:
            f.write(config)
