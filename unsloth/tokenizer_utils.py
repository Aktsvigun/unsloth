...
def try_fix_tokenizer(tokenizer: PreTrainedTokenizerFast, prepend: bool = True) -> PreTrainedTokenizerFast:
	"""
	Attempts to fix a tokenizer by correcting token mappings and ensuring consistency between token names and IDs.
	
	Args:
		tokenizer (PreTrainedTokenizerFast): The tokenizer to be fixed.
		prepend (bool, optional): Whether to prepend a specific token. Defaults to True.
...
def convert_to_fast_tokenizer(
	...
	"""
	Converts a slow tokenizer to a fast tokenizer, ensuring consistency in vocabulary and special tokens.
	
	Args:
		slow_tokenizer (PreTrainedTokenizer): The slow tokenizer to be converted.
		temporary_location (str, optional): The path to a temporary directory for intermediate files. Defaults to "_unsloth_sentencepiece_temp".
	
	Returns:
		PreTrainedTokenizerFast: The converted fast tokenizer.
	"""
...
def assert_same_tokenization(slow_tokenizer: PreTrainedTokenizer, fast_tokenizer: PreTrainedTokenizerFast) -> bool:
	"""
	Checks if two tokenizers produce the same tokenization for a given set of special tokens and a sample text.
	
	Args:
		slow_tokenizer (PreTrainedTokenizer): The slow tokenizer.
		fast_tokenizer (PreTrainedTokenizerFast): The fast tokenizer.
	
	Returns:
		bool: True if both tokenizers produce the same tokenization, False otherwise.
	"""
	# Get eos_token, bos_token etc
...
def fix_sentencepiece_tokenizer(
	...
	"""
	Fixes a SentencePiece tokenizer by updating token mappings in the tokenizer's model file.
	
	Args:
		old_tokenizer (PreTrainedTokenizerFast): The original tokenizer.
		new_tokenizer (PreTrainedTokenizerFast): The new tokenizer with updated token mappings.
		token_mapping (dict): A dictionary mapping old token strings to new token strings.
		temporary_location (str, optional): The path to a temporary directory for intermediate files. Defaults to "_unsloth_sentencepiece_temp".
...
def _load_correct_tokenizer(
	...
	"""
	Loads a tokenizer, attempting to fix any issues with the tokenizer's configuration or vocabulary.
	
	Args:
		tokenizer_name (str): The name or path of the tokenizer.
		model_max_length (int, optional): The maximum length of the tokenized output. Defaults to None.
		padding_side (str, optional): The side on which to pad the tokenized sequences. Defaults to "right".
		token (str, optional): The token to use for authentication. Defaults to None.
		trust_remote_code (bool, optional): Whether to trust remote code execution. Defaults to False.
		cache_dir (str, optional): The directory to cache the tokenizer. Defaults to "huggingface_tokenizers_cache".
		fix_tokenizer (bool, optional): Whether to attempt to fix the tokenizer. Defaults to True.
...
def load_correct_tokenizer(
	...
	"""
	Loads a tokenizer and ensures its chat template is correctly configured.
...
def _find_end_position(template: str, endfor: str, endif: str) -> Optional[str]:
	"""
	Finds the position of the end of a loop or conditional block in a chat template.
	
	Args:
		template (str): The chat template string.
		endfor (str): The string indicating the end of a loop.
		endif (str): The string indicating the end of a conditional block.
	
	Returns:
		Optional[str]: The string indicating the end of the block, or None if not found.
	"""
...
def _fix_chat_template(chat_template: str) -> str:
	"""
	Adjusts a chat template to ensure it correctly handles generation prompts.
	
	Args:
		chat_template (str): The original chat template.
	
	Returns:
		str: The adjusted chat template.
	"""
	endfor = "{% endfor %}"
	endif = "{% endif %}"
...
def fix_chat_template(tokenizer: PreTrainedTokenizerFast) -> Optional[str]:
	"""
	Fixes the chat template of a tokenizer to ensure it correctly handles generation prompts.
	
	Args:
		tokenizer (PreTrainedTokenizerFast): The tokenizer whose chat template needs fixing.
	
	Returns:
		Optional[str]: The fixed chat template, or None if no fix was needed.
	"""
...
def check_tokenizer(
	...
	"""
	Checks a tokenizer for out-of-bounds token IDs and attempts to fix them.
	...
	Args:
		...
		model_name (str, optional): The name of the model. Defaults to "unsloth/llama-2-7b-bnb-4bit".
		model_max_length (int, optional): The maximum length of the tokenized output. Defaults to 4096.
		padding_side (str, optional): The side on which to pad the tokenized sequences. Defaults to "right".
		token (str, optional): The token to use for authentication. Defaults to None.
		_reload (bool, optional): Whether to reload the tokenizer if fixing fails. Defaults to True.
