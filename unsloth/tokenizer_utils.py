...
def try_fix_tokenizer(
    tokenizer: PreTrainedTokenizerFast, prepend: bool = True
) -> PreTrainedTokenizerFast:
    """
    Attempts to fix a tokenizer by correcting inconsistencies in token mappings and special tokens.

    Args:
            tokenizer (`PreTrainedTokenizerFast`):
                    The tokenizer to be fixed.
            prepend (`bool`, *optional*, defaults to `True`):
                    Whether to prepend a special token.
...
def get_sorted_dict(dictionary: dict[str, int]) -> dict[str, int]:
    """
    Returns a dictionary sorted by the values of the original dictionary.
    ...
    Returns:
            `dict[str, int]`: A new dictionary with sorted values.
    """
...
def convert_to_fast_tokenizer(
    slow_tokenizer: PreTrainedTokenizerFast,
    temporary_location: str = "_unsloth_sentencepiece_temp",
) -> PreTrainedTokenizerFast:
    """
    Converts a slow tokenizer to a fast tokenizer while ensuring consistency in vocabulary and special tokens.

    Args:
            slow_tokenizer (`PreTrainedTokenizerFast`):
                    The slow tokenizer to be converted.
            temporary_location (`str`, *optional*, defaults to `"_unsloth_sentencepiece_temp"`):
                    Directory to store temporary files during conversion.
...
def assert_same_tokenization(
    slow_tokenizer: PreTrainedTokenizerFast, fast_tokenizer: PreTrainedTokenizerFast
) -> bool:
    """
    Verifies that two tokenizers produce the same tokenization results for special tokens and chat templates.

    Args:
            slow_tokenizer (`PreTrainedTokenizerFast`):
                    The slow tokenizer to compare.
            fast_tokenizer (`PreTrainedTokenizerFast`):
                    The fast tokenizer to compare.

    Returns:
            `bool`: True if both tokenizers produce the same results, False otherwise.
    """
    # Get eos_token, bos_token etc
...
def fix_sentencepiece_tokenizer(
    ...
    """
    Fixes a SentencePiece tokenizer by updating token mappings in the model file.

    Args:
            old_tokenizer (`PreTrainedTokenizerFast`):
                    The original tokenizer.
            new_tokenizer (`PreTrainedTokenizerFast`):
                    The new tokenizer with updated mappings.
            token_mapping (`dict[str, str]`):
                    Mapping of old tokens to new tokens.
            temporary_location (`str`, *optional*, defaults to `"_unsloth_sentencepiece_temp"`):
                    Directory to store temporary files during fixing.
...
def _load_correct_tokenizer(
    ...
    """
    Loads a tokenizer with options to adjust max length, padding, and token settings.
    ...
    Args:
            ...
            trust_remote_code (`bool`, *optional*, defaults to `False`):
                    Whether to allow remote code execution.
            cache_dir (`str`, *optional*, defaults to `"huggingface_tokenizers_cache"`):
                    Directory to cache the tokenizer.
            fix_tokenizer (`bool`, *optional*, defaults to `True`):
...
def load_correct_tokenizer(
    ...
    """
    Loads and fixes a tokenizer, ensuring chat templates are properly configured.
    ...
    Returns:
            `PreTrainedTokenizerFast`: The loaded and fixed tokenizer.
    """
...
def _find_end_position(template: str, endfor: str, endif: str) -> Optional[str]:
    """
    Finds the position of the end of a loop or conditional block in a template string.

    Args:
            template (`str`):
                    The template string to search.
            endfor (`str`):
                    The end of loop marker.
            endif (`str`):
                    The end of conditional marker.

    Returns:
            `str`: The marker found, or `None` if not found.
    """
...
def _fix_chat_template(chat_template: str) -> str:
    """
    Adjusts a chat template to ensure proper handling of generation prompts.

    Args:
            chat_template (`str`):
                    The original chat template.

    Returns:
            `str`: The fixed chat template.
    """
    endfor = "{% endfor %}"
    endif = "{% endif %}"
...
def fix_chat_template(tokenizer: PreTrainedTokenizerFast) -> Optional[str]:
    """
    Fixes a chat template to ensure compatibility with generation prompts.

    Args:
            tokenizer (`PreTrainedTokenizerFast`):
                    The tokenizer whose chat template needs fixing.

    Returns:
            `Optional[str]`: The fixed chat template or `None` if fixing failed.
    """
...
def check_tokenizer(
    ...
    """
    Validates and fixes a tokenizer to prevent out-of-bounds token IDs.

    Args:
            model (`PeftModelForCausalLM`):
                    The model associated with the tokenizer.
            tokenizer (`PreTrainedTokenizerFast`):
                    The tokenizer to check and fix.
            model_name (`str`, *optional*, defaults to `"unsloth/llama-2-7b-bnb-4bit"`):
                    Name of the model.
            ...
            token (`str`, *optional*):
                    Authentication token for private models.
            _reload (`bool`, *optional*, defaults to `True`):
                    Whether to reload the tokenizer if issues are found.

    Returns:
            `PreTrainedTokenizerFast`: The validated and possibly fixed tokenizer.
    """
